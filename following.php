...
$stmt = $pdo->prepare("
    SELECT u.user_id
    FROM users u
    INNER JOIN follows f ON u.user_id = f.followee_id
    WHERE f.follower_id = ?
            ...
            <?php else: ?>
                <ul class="user-list">
                    <?php foreach ($following as $follow): ?>
                        <li>
                            <a href="dashboard.php?search=user_id:<?php echo urlencode($follow['user_id']); ?>">
                                <?php echo htmlspecialchars($follow['user_id']); ?>
                            </a>
                        </li>
                    <?php endforeach; ?>
                </ul>
